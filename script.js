document.addEventListener('DOMContentLoaded', () => {
    ...
    function setLanguage(lang) { const data = languageData[lang]; if (!data) return; document.querySelectorAll('[data-lang-key]').forEach(el => { const key = el.getAttribute('data-lang-key'); if (data[key]) { if(el.tagName === 'SPAN') { el.textContent = data[key]; } else {el.innerHTML = data[key];}} }); gameTitleH2.innerHTML = data.gameTitle; document.getElementById('fullName').placeholder = data.fullNamePlaceholder; document.getElementById('email').placeholder = data.emailPlaceholder; document.getElementById('socialUser').placeholder = data.socialUserPlaceholder; const label = document.querySelector('label[for="terms-checkbox"]'); if(label) { const link = label.querySelector('a'); const textNode = document.createTextNode(data.labelTerms); label.innerHTML = ''; label.appendChild(link); label.appendChild(textNode); } prizeInfoContainer.innerHTML = `<h4>${data.prizeInfoTitle}</h4><ul><li>${data.level2Prize}</li><li>${data.level4Prize}</li><li>${data.level6Prize}</li></ul><p>${data.dailyLivesInfo}</p>`; document.documentElement.lang = lang; localStorage.setItem('preferredLanguage', lang); }
    function updateLivesDisplay() { livesContainer.innerHTML = ''; for (let i = 0; i < gameState.lives; i++) { const heart = document.createElement('span'); heart.innerHTML = '❤️'; livesContainer.appendChild(heart); }}
    function showOverlay(title, message, buttonText, buttonAction) { overlayTitle.innerHTML = title; overlayMessage.innerHTML = message; overlayButton.style.display = 'block'; overlayButton.innerHTML = buttonText; overlayButton.onclick = () => { gameOverlay.classList.add('hidden'); if(buttonAction) buttonAction(); }; overlayPrizeInfo.innerHTML = ''; gameOverlay.classList.remove('hidden'); }
    function showPrizeScreen(level) { const lang = document.documentElement.lang; const prizeValue = level.prize[lang]; const fullName = document.getElementById('fullName').value; const email = document.getElementById('email').value; overlayTitle.innerHTML = getText('congrats'); overlayMessage.innerHTML = getText('prizeClaimMsg'); overlayPrizeInfo.innerHTML = `<h3>${prizeValue}</h3>`; const prizeButton = document.createElement('a'); prizeButton.className = 'prize-button'; prizeButton.innerHTML = getText('claimPrize'); const emailSubject = `${getText('emailSubject')} ${level.level}`; const emailBody = `${getText('emailBody').replace('[Seviye]', `Seviye ${level.level}`)}\n\nAd Soyad: ${fullName}\nE-posta: ${email}`; prizeButton.href = `mailto:giveaways@kyrosil.eu?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(emailBody)}`; overlayPrizeInfo.appendChild(prizeButton); overlayButton.innerHTML = getText('nextLevel'); overlayButton.style.display = 'block'; overlayButton.onclick = () => { gameOverlay.classList.add('hidden'); gameState.currentLevel++; saveGameState(); loadLevel(gameState.currentLevel); }; gameOverlay.classList.remove('hidden'); }
    ...
    function loadLevel(levelIndex) {
        try {
            ...
            if (!level) { showOverlay(getText('congrats'), getText('allLevelsComplete'), getText('restartGame'), () => { gameState.currentLevel=0; gameState.lives=3; saveGameState(); window.location.reload(); }); return; }
            levelTitle.innerHTML = `${getText('level')} ${level.level}: ${getText(`level${level.level}Name`)}`;
            updateLivesDisplay();
            ...
            level.placeholders.forEach(p => { const placeholder = document.createElement('div'); placeholder.className = 'placeholder'; placeholder.style.left = p.x + 'px'; placeholder.style.top = p.y + 'px'; placeholder.style.width = p.w + 'px'; placeholder.style.height = p.h + 'px'; placeholder.dataset.accepts = p.accepts; mainPartDiv.appendChild(placeholder); });
            ...
            console.error("Seviye yüklenirken bir hata oluştu:", error);
            alert("Oyun yüklenirken beklenmedik bir hata oluştu. Lütfen sayfayı yenileyin.");
        }
    }
    ...
    function checkWinCondition() { const level = levelData[gameState.currentLevel]; const totalPartsNeeded = level.placeholders.length; const placedParts = assemblyTarget.querySelectorAll('.placeholder .furniture-part').length; if (placedParts === totalPartsNeeded) { setTimeout(handleLevelWin, 500); } }
    function handleLevelWin() { const level = levelData[gameState.currentLevel]; if (level.prize) { showPrizeScreen(level); } else { const msg = `${getText('level')} ${level.level} ${getText('isComplete')}`; showOverlay(getText('levelComplete'), msg, getText('nextLevel'), () => { gameState.currentLevel++; saveGameState(); loadLevel(gameState.currentLevel); }); } }
    ...
    entryForm.addEventListener('submit', e => { e.preventDefault(); if (!termsCheckbox.checked) { alert(getText('termsTitle') + getText('mustBeAccepted')); return; } startGame(); });
