document.addEventListener('DOMContentLoaded', () => {
    ...
    // ---------- 3. OYUN SEVİYE VERİLERİ (ZORLAŞTIRILDI) ----------
    const levelData = [
        { level: 1, prize: null, productName: "LACK Sehpa", mainPart: { w: 300, h: 300, class: 'part-wood' }, parts: [{id:'leg', q:4, w:40, h:100, class:'part-wood'}, {id:'fakeLeg', q:1, w:40, h:100, class:'part-wood'}], placeholders: [{x:10,y:10,w:40,h:40,accepts:'leg'},{x:250,y:10,w:40,h:40,accepts:'leg'},{x:10,y:250,w:40,h:40,accepts:'leg'},{x:250,y:250,w:40,h:40,accepts:'leg'}] },
    ...
    function checkWinCondition() { const level = levelData[gameState.currentLevel]; const totalParts = level.parts.reduce((sum, part) => { return part.id.startsWith('fake') ? sum : sum + part.q; }, 0); const placedParts = assemblyTarget.querySelectorAll('.furniture-part').length; if (placedParts === totalParts) { setTimeout(handleLevelWin, 500); } }
