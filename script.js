document.addEventListener('DOMContentLoaded', () => {
    ...
    // ** DÜZELTİLMİŞ SEVİYE YÜKLEME FONKSİYONU **
    function loadLevel(levelIndex) {
        gameOverlay.classList.add('hidden');
        const level = levelData[levelIndex];
        if (!level) {
            showOverlay(getText('congrats'), getText('allLevelsComplete'), getText('restartGame'), () => {
                gameState.currentLevel=0; gameState.lives=3; saveGameState(); window.location.reload(); 
            });
            return;
        }
        ...
        // Reset placeholder usage flag for re-levels
        level.placeholders.forEach(p => p.isUsed = false);

        let mainPartDiv;
        if (level.mainPart.isWall) {
            mainPartDiv = assemblyTarget;
        } else {
            mainPartDiv = document.createElement('div');
        ...
        level.placeholders.forEach(p => {
            ...
            placeholder.dataset.label = p.label;
        ...
        level.parts.forEach(part => {
            for(let i=0; i<part.q; i++) {
                ...
                const placeholderForThisPart = level.placeholders.find(p => p.accepts === part.id && !p.isUsed);

                if (placeholderForThisPart) {
                    partEl.dataset.label = placeholderForThisPart.label;
                    placeholderForThisPart.isUsed = true; // Mark as used for this part instance
                } else {
                    partEl.dataset.label = 'X'; // It's a fake part
                }
        ...
        // Reset the usage flag again after creating parts, for the next level load
        level.placeholders.forEach(p => p.isUsed = false);
    ...
    function addDragDropListeners() {
        ...
        document.querySelectorAll('.placeholder').forEach(placeholder => {
            ...
            placeholder.addEventListener('drop', e => {
                e.preventDefault();
                e.stopPropagation(); // Prevent the event from bubbling up to assemblyTarget
